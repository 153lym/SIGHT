...
class Encoder(Module):
    def __init__(self, in_features, out_features, graph_neigh, dropout=0.0, act=F.relu):
        ...
        # INR
        mid_channel = 200
        self.encoder_layer0 = nn.Sequential(SineLayer(2, mid_channel),
                                            SineLayer(mid_channel, mid_channel),
                                            SineLayer(mid_channel, 30),
                                            DenseLayer(30, self.in_features))

        self.encoder_layer1 = DenseLayer(self.in_features, self.in_features)

        self.decoder = nn.Sequential(SineLayer(self.in_features, mid_channel),
                                     DenseLayer(mid_channel, self.in_features))
    ...
    def forward(self, feat, feat_a, adj,coord):
        ...
        # INR
        coord = coord / 100  # Scale the coordinates
        mid_fea = self.encoder_layer0(coord)
        inr_Z = self.encoder_layer1(mid_fea)
        node_feats_recon = self.decoder(inr_Z)
        ...
        return hiden_emb, h,mid_fea, node_feats_recon, ret, ret_a, g, g_a, emb, emb_a
